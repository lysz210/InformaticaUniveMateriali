...
function startGame(svg)
{
    ...
    window.addEventListener("mousemove",
        function(event)
        {
            if(!currentPole.a.isNull()) {
                var poleb = new Vector(event.x - svg.offsetLeft, event.y - svg.offsetTop);
                socket.emit('pole_update', {
                    a: currentPole.a,
                    b: poleb }
                );
            }
        ...
        function(event)
        {
            if(!currentPole.a.isNull())
            {
                if(currentPole.len()>0) {
                    socket.emit("ball_fired", {
                        speed: currentPole.a.clone().sub(currentPole.b).mul(0.005)
                    });
                }
            }
        });
    ...
    socket.on("pole_update", function(obj) {
        currentPole.a.x = obj.a.x;
        currentPole.a.y = obj.a.y;
        currentPole.b.x = obj.b.x;
        currentPole.b.y = obj.b.y;
    });

    socket.on("ball_fired", function(obj) {
        white.speed.x = obj.speed.x;
        white.speed.y = obj.speed.y;
        currentPole.reset();
    });
    ...
    var game_update = function( elapsed ) {
